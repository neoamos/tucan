defmodule App.StorageTest do
  ...
  test "store event", %{seckey: s, pubkey: p, event1: event1_json} do

    # Basic storing event
    {:ok, event} = Event.create("asdf", s, 1, [["t", "test"]])
    Storage.store(event, relays: ["wss://relay.example.org"])
    es = Storage.get_event({:event_id, event.id})
    retrieved_event = Storage.event_from_storage(es)
    assert event == retrieved_event

    # Adding new relays
    Storage.store(event, relays: ["wss://relay.example2.org"])
    es = Storage.get_event({:event_id, event.id}, [:relays, relays: [:relay]])
    assert Enum.any?(es.relays, fn er -> er.relay.url == "wss://relay.example.org" end)
    assert Enum.any?(es.relays, fn er -> er.relay.url == "wss://relay.example2.org" end)

    # Storing external event
    {:ok, event} = Event.create(Jason.decode!(event1_json))
    Storage.store(event)
    es = Storage.get_event({:event_id, event.id})
    retrieved_event = Storage.event_from_storage(es)
    assert event == retrieved_event
    assert Event.valid?(retrieved_event)
  end
